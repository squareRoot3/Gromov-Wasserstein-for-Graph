...
save_name = "data/amazon_noise.txt"
...
def process_sgwl_amazon(cost,database,num_nodes,num_partitions,verbose=False):
...
# mutual_info,_,_ = process_sgwl_amazon(cost,database,num_nodes,num_partitions);
# # mutual_info,_,_ = process_sgwl_amazon(cost,database,num_nodes,num_clusters_raw_sym);
# end = time.time()
...
# mutual_info,_,_ = process_sgwl_amazon(cost,database,num_nodes,num_partitions);
# # mutual_info,_,_ = process_sgwl_amazon(cost,database,num_nodes,num_clusters_noisy_sym);
# end = time.time()
...
def t_selection_pipeline_undirected_amazon(G,ts,num_partitions,fraction_t_to_keep=0.25):
    ...
    for t in ts:
        ...
        mutual_info, d_gw, coup = process_sgwl_amazon(cost,database,num_nodes,num_partitions)
...
for j in num_clusts:
    mutual_info, d_gw, coup = process_sgwl_amazon(cost,database,num_nodes,j)
    partition = get_partition(coup)
    mis.append(mutual_info)
    d_gws.append(d_gw)
    coverages.append(coverage(G,partition))
...
ts = np.linspace(100,150,20)
mis, coups, d_gws, good_t_max, good_t_grad, rt = t_selection_pipeline_undirected_amazon(G,ts,estimated_clusters_raw_sym)
...
amazon_raw_sym_ami = mis[np.argmax(coverages)]
print('AMI for AMAZON, Raw, Sym:',amazon_raw_sym_ami)
print('Occurs at t-value:',ts[np.argmax(coverages)])
scores['specgwl-symmetric-raw'] = amazon_raw_sym_ami
...
mis, coups, d_gws, good_t_max, good_t_grad, rt = t_selection_pipeline_undirected_amazon(nG,ts,estimated_clusters_noisy_sym)
...
amazon_noisy_sym_ami = mis[np.argmax(coverages)]
print('AMI for AMAZON, Noisy, Sym:',amazon_noisy_sym_ami)
print('Occurs at t-value:',ts[np.argmax(coverages)])
scores['specgwl-symmetric-noisy'] = amazon_noisy_sym_ami
...
num_partitions_clean = 25
t = 102.63157894736842
num_partitions_noise = 23
t_noise = 20
